...
import { AuthSig, CapabilityObject, CapabilityProtocolPrefix, SessionSig, SessionSigSignedMessage } from '../models';
...
function parseSIWEMessage(siweMessage: string): [SiweMessage | null, Error | null] {
    try {
...
export function getFullResourceUri(protocolPrefix: CapabilityProtocolPrefix, resourceUri: string): string {
...
export async function validateSessionSignature(
    ...
    if (!!parseRes[1]) {
        return ["", new Error(`Unable to parse session sig signed message: ${parseRes[1]}`)];
    }
    ...
    try {
        const [creatorAddress, validateCapabilityErr] = await validateSessionCapability(
        ...
        if (!!validateCapabilityErr) {
            return ["", new Error(`Invalid capabilities array: ${validateCapabilityErr}`)];
        }
    
        return [creatorAddress, null];
    } catch (validationErr: any) {
        return ["", new Error(`Unable to validate capabilities: ${validationErr.toString()}`)];
    }
}
...
async function validateSessionCapability(
    ...
    for (let i = 0; i < capabilities.length; i++) {
        ...
        if (!!parseRes[1]) {
            return ["", new Error(`Unable to parse session sig SIWE message: ${parseRes[1]}`)];
        }
    ...
    for (let i = 0; i < siweResources.length; i++) {
        ...
        if (capabilityObject.def) {
            for (const defaultAction of capabilityObject.def) {
                if (defaultAction == capabilityProtocolPrefix.toString() || defaultAction === "*") {
                    return null;
                }
            }
        }
        ...
        if (capabilityObject.tar) {
            ...
            for (let j = 0; j < tarKeys.length; j++) {
                ...
                const isActionPermitted = permittedActions.indexOf(capabilityProtocolPrefix.toString()) > -1 ||
                                            permittedActions.indexOf("*") > -1;
                if (resourceIdHash === "*" && isActionPermitted) {
                    return null;
                } else if (resourceIdHash === requestedHashedResourceId && isActionPermitted) {
                    return null;
                }
            }
        }
