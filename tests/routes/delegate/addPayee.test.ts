...
describe('addPayeeHandler Load Test', () => {
    ...
    beforeAll(async () => {
        if (!API_KEY || !PAYER_SECRET) {
            throw new Error('TEST_LIT_RELAYER_API_KEY and TEST_LIT_PAYER_SECRET_KEY must be set in .env file');
        }
        
        // Log the test wallet address so it can be funded
        const { deriveWallet } = await import('../../../routes/delegate/register');
    ...
    it('should handle concurrent requests without nonce collisions', async () => {
        ...
        const results: { success: boolean; error?: any; index: number; isNonceError?: boolean; data?: any }[] = [];
        
        console.log(`Starting load test with ${numRequests} parallel requests...`);
        ...
        if (failed.length > 0) {
            ...
            console.log(`\nNonce error rate among failures: ${(nonceErrorRate*100).toFixed(1)}%`);
