...
import {
	...
	StoreConditionRequest,
	StoreConditionResponse,
} from "../models";
...
export async function storeConditionHandler(
	...
	if (
		req.body.capabilityProtocolPrefix !==
			CapabilityProtocolPrefix.LitEncryptionCondition.toString() &&
		req.body.capabilityProtocolPrefix !==
			CapabilityProtocolPrefix.LitSigningCondition.toString()
	) {
		return res.status(400).json({
			error: `Only the following capability protocol prefixes are supported: ${[
				CapabilityProtocolPrefix.LitEncryptionCondition,
				CapabilityProtocolPrefix.LitSigningCondition,
			]}`,
		});
	}
...
function validateRequest(requestBody: StoreConditionRequest): Error | null {
	const keysToCheckExist: Array<keyof StoreConditionRequest> = [
		"key",
		"value",
		"securityHash",
		"chainId",
	...
	const keysToCheckByteStringLength: Array<
		keyof Pick<StoreConditionRequest, "key" | "value" | "securityHash">
	> = ["key", "value", "securityHash"];
